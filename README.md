...
Keyball39の基板はPro Microソケット（コンスルー）に裏返しで挿す構造上、組み込んだ状態だとBOOTSELボタンに指が届きにくい。そのため書き込みは2段階で行う。

### 初回書き込み（基板単体で）

キーボードに挿し込む**前**に、RP2040単体にUSBケーブルを直接挿して書き込む。

1. SparkFun Pro RP2040にUSBケーブルを挿す前に、BOOTSELボタン（ラベル無し。リセットピン側がRESET、逆側がBOOTSEL）を押しながらUSB接続する
...
5. 書き込みが終わったら、Keyball39のコンスルーに挿し込んで組み込む

### 2回目以降の再書き込み（組み込んだ状態のまま）

このファームウェアはQMKの「ダブルタップリセット」機能が有効（`RP2040_BOOTLOADER_DOUBLE_TAP_RESET`、QMK_PM2040ボードのデフォルト設定）になっているため、組み込んだ後の再書き込みはRP2040本体のBOOTSELボタンを探す必要はない。

1. Keyball39基板上のリセットボタン（SW19）を**0.5秒以内に素早く2回押す**
2. ブートローダーモードに入り、「RPI-RP2」ドライブが出現する
3. 以降は初回と同じく`.uf2`をコピーする

うまく入らない場合は、コンスルーからRP2040を一時的に外して基板単体でBOOTSELボタンを押す方法に切り替える（コンスルー接続なのでハンダ付け不要で着脱できる）。
