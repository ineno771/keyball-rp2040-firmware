...
// あえて明記しておく。Keyball+はKeyball39と基板配線が共通（マトリクス・SPIとも
// 同一ピン）のため、keyball39/config.hの値をそのまま使用している。
#define SPI_SCK_PIN          B1
#define SPI_MOSI_PIN         B2
#define SPI_MISO_PIN         B3

// RGB LED settings
// Keyball+はLED数が左右非対称（トラックボール搭載側26個・非搭載側29個、合計55個）。
// 値はAVR版keyball-plus-firmwareのconfig.hから流用（実機で確認済みの値）。
#define WS2812_DI_PIN       D3
#ifdef RGBLIGHT_ENABLE
#    define RGBLIGHT_LED_COUNT  55
#    define RGBLED_SPLIT        { 29, 26 }
