...
- **トラックボール「矢印キーモード」構想（アイデア段階、未着手）**: トラックボールが回転している間だけ、その方向の矢印キーを押しっぱなしにするモード。ゼロ点（基準位置）は不要で、毎回のセンサー移動量から「今どちら向きにどれくらいの速さで動いているか」を見て閾値判定する方式。
  - 決定済み仕様: (1) 誤入力防止のデッドゾーン（閾値）を設ける。(2) 斜め入力は無し。上下・左右いずれか強く動いている軸だけを採用する。
  - 既存のジェスチャー機能（`GESTURE_ENABLE`、トラックボール移動量を累積して方向判定するロジック）と技術的に近く、流用しやすい見込み。
- **トラックボール「超低速（精密作業）モード」構想（アイデア段階、未着手）**: 対応キーを押している間だけトラックボールの移動量を大きく下げるモード。細かい作業（ピクセル単位の操作等）向け。
  - 既存のCPI（感度）ランタイム変更の仕組みを流用できる見込み。キー押下時に一時的にCPIを下げ、離したら元に戻す処理を追加するだけで実現できそう。
  - 低速時の倍率（どれくらい遅くするか）は固定値ではなく**Keyball Linkから調整できるようにしたい**とのこと（本人希望）。Keyball Link側の対応も必要になる。keyball-link側にも記録済み（`~/keyball-configurator/HANDOFF.md`参照）。
