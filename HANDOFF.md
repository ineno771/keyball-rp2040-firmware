...
- [ ] **コミット未実施（2026-09-11時点）**: 本セッションの変更は全て`keyball-rp2040-firmware`・`keyball-configurator`ともに**未コミット**（作業ディレクトリに変更が残った状態）。次回セッション開始時、まず`git status`で差分を確認すること。コミットは必ず内容を提案してから本人の明確な許可を得て実行する（標準ルール、1章参照）。
- [x] **Keyball+のRP2040移植を開始（2026-09-14、ビルド確認のみ・実機未確認）**: `keyboards/keyball/keyballplus/`を新規追加。Keyball39 RP2040版をベースに、AVR版keyball-plus-firmwareとの差分（LED数55個・左右非対称29/26、USB識別情報）だけを適用する方針で移植した。
  ...
  - **LED物理位置（`g_led_config`、`keyballplus.c`）は暫定値・未校正**: マトリクス→LEDインデックスの対応、LEDインデックス→物理座標(x,y)とも、Keyball39実機で行ったような「1個ずつ点灯確認する校正作業」をKeyball+ではまだ行っていない。現状は機械的にキー位置順へ番号を振っただけの仮データ。基本機能（マトリクス・トラックボール・OLED・単色系LED演出）はこのままで動作するはずだが、押した位置に反応する演出（リアクティブ・リップル・タイピングヒートマップ・ジェスチャーウェーブ等）は実機到着後、Keyball Linkの「LED位置実測（開発用）」機能で校正するまで正しく対応しない可能性が高い。
