...
- **トラックボール「超低速（精密作業）モード」（実装済み・2026-08-25）**: 新規キーコード`PRC_MO`（`keyball.h`）を押している間、CPIを分周値（既定4、範囲2-20、`kb_precision_div_get/set`でEEPROM保存）で割って移動量を下げる。離すと元のCPIに復帰。
  - 分周値はKeyball Linkから調整できるよう、新規HIDコマンド`KB_HID_CMD_GET_PRECISION`(0x18)/`KB_HID_CMD_SET_PRECISION`(0x19)を追加済み（`kb_hid.h`/`kb_hid.c`）。**Keyball Link側のUI実装はまだ**（`~/keyball-configurator/HANDOFF.md`に記録済みのタスク）。
  - `PRC_MO`はデフォルトのキーマップ配列には未割り当て。Keyball Link側でキー割り当てUIが対応してから、ユーザーが任意のキーに割り当てる想定。
