...
- **（不採用）クリック時刻の位置スムージング**: クリック直前のセンサー履歴を遡って座標を補正する案を検討したが、標準的なUI要素に対しては`SteadyPoint`（旧keyball-companion。別プロジェクト、https://github.com/ineno771/steady-point ）側の「磁石的吸着＋吸着中フリーズ」機能で同等の効果が得られるため不採用。自由座標（キャンバス等）への精密クリック用途では価値が残る可能性はあるが、現時点では見送り。
- **LED「波紋」演出構想（アイデア段階、未着手）**: キーを押した位置から波紋のように光が広がるLEDアニメーション。**LEDの光り方設定の1つとして選択可能にする**（常時ONの固定演出ではなく、他のエフェクトと同様に設定で選べるようにする）。
- **レイヤー切替通知LED演出構想（アイデア段階、未着手）**: レイヤーが切り替わった瞬間に、切り替わったことが分かる一時的なLED演出（フラッシュ・パルス等）を入れる。常時表示のレイヤー番号インジケーターではなく、**切り替えの瞬間だけ光る通知的な演出**（本人希望・確認済み）。
...
- [ ] **RP2040の大容量フラッシュを活かした新機能**: RGB_MATRIX移行・マクロ大容量化。トラックボールジェスチャー・LED・OLED関連の詳細なアイデアは2章に一覧化済み（矢印キーモード仕様変更、LED波紋演出、レイヤー切替LED演出、OLEDリッチ化、ジェスチャーレイヤー切替、汎用連続値調整機能、スクロール慣性、レイヤー数8化）。
