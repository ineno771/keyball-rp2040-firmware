...
- **（不採用）クリック時刻の位置スムージング**: クリック直前のセンサー履歴を遡って座標を補正する案を検討したが、標準的なUI要素に対しては`keyball-companion`側の「磁石的吸着＋吸着中フリーズ」機能で同等の効果が得られるため不採用。自由座標（キャンバス等）への精密クリック用途では価値が残る可能性はあるが、現時点では見送り。
